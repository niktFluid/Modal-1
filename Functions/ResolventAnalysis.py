...
class ResolventMode(FieldData):
    def __init__(self, mesh, operator, n_val=5, k=10, sigma=None, which='LM'):
        self.operator = operator
        self._arpack_ops = {'k': k, 'sigma': sigma, 'which': which}
        ...
        super(ResolventMode, self).__init__(mesh, n_val=n_val * k, data_list=data_list)

    def _init_field(self, *args, **kwargs):
        k = self._arpack_ops['k']
        self.data = np.empty((self.n_cell, self._n_values * k), dtype=np.float64)

    def solve(self):
        operator = self._set_operator()
        vecs_l, svs, vecs_r = linalg.svds(self.operator, **self._arpack_ops)
        self.eigs = eigs
